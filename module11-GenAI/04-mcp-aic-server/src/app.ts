...
server.registerTool(
  ...
  async ({ query, page = 1 }) => {
    try {
      const size = 10;
      const from = (page - 1) * size; // (1-1) * 10 = 0  // 2 = 10 // 3 = 2*size
      ...
      return {
        ...
        isError: true, // Makes it clear this is an error response
      };
    }
  },
);
...
server.registerPrompt(
  'artwork_search_prompt',
  {
    ...
    description: 'A prompt to search for artworks using a query and page number.',
    argsSchema: {
      query: z.string().describe('Search query for artworks'),
    },
  },
